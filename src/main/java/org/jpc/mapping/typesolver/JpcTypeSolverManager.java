...
public class JpcTypeSolverManager extends TypeSolverManagerImpl {
	...
	public JpcTypeSolverManager(JCategory categorization) {
		this(categorization, new JpcEngine());
	}
	
	public JpcTypeSolverManager(JCategory categorization, JpcEngine embeddedEngine) {
	...
	private Type evalQuantifiedTermTypeSolver(Term term) {
		...
		if(isValidTypeSolvableTerm(term)) {
			...
			while(query.hasNext()) {
				...
				} catch(UnrecognizedObjectException e) {} //just try with the next type solver.
			}
			query.close();
		}
		if(type == null)
