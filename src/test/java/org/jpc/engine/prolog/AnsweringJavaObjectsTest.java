...
public class AnsweringJavaObjectsTest {
	...
	public void testObjectReference() {
		...
		Query query = getPrologEngine().query(new Compound("=", Arrays.<Term>asList(new Var("A"), JTermManager.weakJTerm(s1))));
	...
	public void testObjectSerialization() {
		...
		Query query = getPrologEngine().query(new Compound("=", Arrays.<Term>asList(new Var("A"), SerializedTerm.serialize(s1))));
	...
	public void testQueryArguments() {
		String s1 = new String("x");
		Query query = getPrologEngine().query("A = ?1/m, B = ?1, C = ?1/r, D = ?1/s", asList(s1));
